...
template<typename T>
struct format_str_helper;

template<>
struct format_str_helper<uint8_t> {
  operator const char*() { return "%3d "; }
};
template<>
struct format_str_helper<float> {
  operator const char*() { return "%6.2f "; }
};

template<typename T>
const char* format_str() {
  return (format_str_helper<T>());
}

template<typename T>
void print_image_by_layer(Image<T> im, std::string filename) {
    FILE *f = fopen(filename.c_str(), "w");
    _assert(f, "File %s could not be opened for writing\n", filename.c_str());
    int layers = (im.dimensions() > 3) ? im.extent(3) : 1;
    fprintf(f, "%d layers, %d channels, %d x %d\n", layers, im.channels(), im.width(), im.height());

    T p;
    for (int d = 0; d < layers; d++) {
        for (int c = 0; c < im.channels(); c++) {
            fprintf(f, " layer %d channel %d\n", d,c);
            for (int y = 0; y < im.height(); y++) {
                for (int x = 0; x < im.width(); x++) {
                    convert(im(x, y, c, d), p);
                    fprintf(f, format_str<T>(), p);
                }
                fprintf(f, "\n");
            }
            if (c < im.channels()-1) fprintf(f, "\n");
        }
        if (d < layers-1) fprintf(f, "\n");
    }
    fclose(f);
}

template<typename T>
void print_image(Image<T> im, std::string filename) {
    /*FILE *f = fopen(filename.c_str(), "w");
    _assert(f, "File %s could not be opened for writing\n", filename.c_str());
    fprintf(f, "%d x %d\n", im.width(), im.height());
    ...
    fclose(f);*/
    print_image_by_layer(im, filename);
}
