...
Func convolutional_layer(Func input, vector<Expr> &data_dim, Image<T> &weight,
  ...
  result(x, y, c, n) = bias(c) + sum(weight(weight.extent(0)-1-r.x,
					 weight.extent(1)-1-r.y, r.z, c) *
				  input(x+weight.extent(0)-1-r.x,
					y+weight.extent(1)-1-r.y, r.z, n));
...
Func innerproduct_layer(Func input, vector<Expr> &data_dim, Image<T> &weight,
  ...
  if(data_dim.size() == 3) {
    ...
    result(x, n) = sum(weight(x, r.x + data_dim[0]*r.y + 
   			   data_dim[0]*data_dim[1]*r.z, 0, 0) *
      		    input(r.x, r.y, r.z, n)) + bias(x);
  } else {
    RDom r(0, data_dim[0]);
    result(x, n) = sum(weight(x, r.x, 0, 0) * input(r.x, n)) + bias(x);
  }
